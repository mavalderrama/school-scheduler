...
CREATE TABLE notifications_log (
  id               BIGSERIAL PRIMARY KEY,
  kind             TEXT NOT NULL,     -- 'daily','gap_check','nudge_empty','reminder'
  reminder_id      BIGINT REFERENCES reminders(id),  -- solo cuando kind='reminder'
  target_date      DATE,
  chat_id          BIGINT NOT NULL,
...
-- Fase 10: avisos a una hora que pide el usuario. No es agenda: no habla de un día del
-- colegio, habla de una hora a la que sonar. `next_fire_at` es la próxima vez ya calculada;
-- el barrido de cada minuto la reserva antes de enviar, así que la entrega es como mucho una.
CREATE TABLE reminders (
  id               BIGSERIAL PRIMARY KEY,
  child_id         BIGINT NOT NULL REFERENCES children(id),
  chat_id          BIGINT NOT NULL,   -- se entrega donde se pidió
  text             TEXT NOT NULL,
  time_of_day      TIME NOT NULL,     -- hora local del colegio del niño
  repeat           TEXT NOT NULL,     -- 'once' | 'daily' | 'weekly'
  weekdays         TEXT NOT NULL DEFAULT '',  -- ISO "135" = lunes, miércoles y viernes
  on_date          DATE,              -- solo con repeat='once'
  only_school_days BOOLEAN NOT NULL DEFAULT false,
  next_fire_at     TIMESTAMPTZ,       -- NULL: ya no vuelve a sonar
...
- Red de seguridad de lo recurrente (Fase 10.3): «los viernes» y «el viernes» se diferencian en un artículo y el modelo resolvió el plural como fecha. `chat.recurring_days()` lo decide en Python y corrige la clasificación; decir después «eso es recurrente» convierte la entrada existente, y las entradas sueltas que la regla cubre se listan en la confirmación y se van con el ✅.
