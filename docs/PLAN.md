...
**No-objetivos (v1):** multi-niño, multi-colegio, OCR de PDFs largos, voz, autenticación de usuarios más allá de la whitelist, interfaz web.
...
| Base de datos | **PostgreSQL 16** | Consultas por rango de fechas, integridad referencial, versionado de entradas. Mongo sería viable pero no aporta nada aquí |
...
Un solo proceso `bot`. Ollama corre aparte. Postgres aparte (mismo compose). El binario de Claude Code, si se usa `claude_sdk`, vive dentro de la imagen del bot.
...
├── alembic/
├── app/
│   ├── main.py              # arranque: bot + scheduler
...
DATABASE_URL=postgresql+asyncpg://agenda:agenda@postgres:5432/agenda
DATA_DIR=/data
TZ=America/Bogota
...
- Docker + Compose plugin. `docker-compose.yml` con servicios `bot` y `postgres` (`postgres:16-alpine`), volúmenes `pgdata` y `./data` (fotos), `restart: unless-stopped`, healthcheck en postgres y `depends_on: condition: service_healthy`.
- El bot **no publica puertos**. Solo sale a `api.telegram.org`, a Ollama (red interna) y, si aplica, a `api.anthropic.com`.
...
- Alembic con la migración 001 (tablas de la sección 5).
