...
- `add_recurring`: dicen que algo se repite **todas las semanas** y no dan hora ("todos los viernes tiene natación", "los martes lleva uniforme de deporte", "cada miércoles hay refuerzo"). Rellena `weekdays` (ISO) y `text` con la actividad, sin los días. No es `add_entry`: ahí hay una fecha; aquí hay un día que vuelve cada semana.
...
11. `add_entry`, `add_recurring` y `add_reminder` se distinguen por **la hora y la repetición**, no por el verbo:
    ...
    - No hay hora y se repite cada semana ("todos los viernes", "los martes y jueves", "cada miércoles") → `add_recurring`. Queda en el horario.
...
13. `weekdays` es ISO: 1 lunes … 7 domingo. Sirve tanto para `add_recurring` como para `repeat` = `weekly`. "entre semana" es `[1,2,3,4,5]`; "todos los días" en un `add_recurring` también es `[1,2,3,4,5]`, que son los días de colegio.
