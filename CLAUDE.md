...
- **Fase 4:** robustez y `/estado`. **Fase 5 (opcional):** Home Assistant.
...
**Comandos sin LLM** (`handlers/commands.py`): `/hoy`, `/manana` (formato de la notificación, sin registrarla), `/semana` (de hoy al domingo; en fin de semana ya mira a la siguiente, `chat.week_range`), `/pendiente`, `/ayuda`, `/ping`. Son la red de seguridad cuando la IA está caída y no deben depender nunca de un proveedor.
